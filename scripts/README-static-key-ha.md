...
| 规模 | 管理密钥数 | 并发请求 | 推荐配置 (每节点) | 适用场景 |
|------|----------|---------|-------------------|----------|
| **小型** | < 1,000 | < 100 QPS | 2C / 4G / 50G SSD | 开发测试、小型团队 |
| **中型** | 1,000 ~ 10,000 | 100 ~ 1,000 QPS | 4C / 8G / 100G SSD | 部门级生产环境 |
| **大型** | 10,000 ~ 100,000 | 1,000 ~ 5,000 QPS | 8C / 16G / 200G NVMe SSD | 公司级生产环境 |
| **超大型** | > 100,000 | > 5,000 QPS | 16C / 32G / 500G NVMe SSD | 平台级、多租户 |

### 资源说明

| 资源 | 说明 | 建议 |
|------|------|------|
| **CPU** | Raft 共识、加解密运算、请求处理 | ≥ 2 核，中型以上推荐 4 核+ |
| **内存** | Raft 日志缓存、密钥缓存、连接池 | ≥ 4 GB，OpenBao 本身约占 100~300MB |
| **磁盘** | Raft 日志 + 快照 + 审计日志 | **必须 SSD**，Raft 对磁盘延迟敏感 |
| **网络** | 集群间通信 (:8201) + 客户端请求 | ≥ 1Gbps，节点间延迟 < 10ms |
| **操作系统** | Linux (amd64/arm64) | Ubuntu 22.04+ / CentOS 8+ / RHEL 8+ |

### 磁盘要求详解

```
OpenBao Raft 存储目录 (/opt/openbao/raft) 磁盘 IO 特征:

┌─────────────────────────────────────────────────────────┐
│ 写入模式: 顺序写 (Raft log append)                      │
│ 写入延迟: 直接影响写请求响应时间                          │
│ 写入量:   每次写操作 = 1 次 Raft log 追加               │
│ 快照:     每 120s 检查一次, 8192 条后触发              │
│ 快照大小: 与存储的密钥/策略/Token 数量成正比              │
│                                                         │
│ 推荐: NVMe SSD > SATA SSD > HDD (不推荐)            │
│ IOPS:   中型 ≥ 3000,  大型 ≥ 10000                   │
│ 延迟:   中型 < 1ms,   大型 < 0.5ms                     │
└─────────────────────────────────────────────────────────┘
```
...
```
客户端 ←──(TCP 8200)──→ ELB ←──(TCP 8200)──→ OpenBao 节点
                              (HTTP, TLS 在 ELB 终止)

节点间 ←──(TCP 8201)──→ OpenBao 节点
                         (Raft 复制 + 请求转发, TLS 加密)

要求:
  ● 节点间网络延迟 < 10ms (同可用区)
  ● 节点间网络延迟 < 50ms (跨可用区, 会增加写延迟)
  ● 防火墙开放: 8200 (API) + 8201 (集群)
  ● 建议使用内网/专线通信
```

### 云服务器推荐配置

| 云厂商 | 小型 (2C4G) | 中型 (4C8G) | 大型 (8C16G) |
|---------|-------------|-------------|-------------|
| **阿里云** | ecs.c7.large | ecs.c7.xlarge | ecs.c7.2xlarge |
| **腾讯云** | S6.LARGE4 | S6.LARGE8 | S6.2XLARGE16 |
| **华为云** | c7.large | c7.xlarge | c7.2xlarge |
| **AWS** | c7g.large | c7g.xlarge | c7g.2xlarge |

> **实例类型选择**: 计算优化型 (C 系列) 优于通用型，因为 OpenBao 主要消耗 CPU 和内存，而非磁盘密集型。

### 部署拓扑建议

```
推荐: 3 节点部署在同一地域的不同可用区
...
优势:
  ● 任意 1 个 AZ 宕机不影响服务 (Raft 仍需 2/3 quorum)
  ● 跨 AZ 延迟通常 < 5ms，不影响 Raft 性能
  ● ELB 自动剔除不健康节点
```
